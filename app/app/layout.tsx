...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Mark dashboard as shown once it's been visible
  useEffect(() => {
    if (isAuthenticated && !isLoginSyncing && !isBackgroundSyncing && !isManualRefresh && photoReady) {
      hasShownDashboard.current = true
    }
  }, [isAuthenticated, isLoginSyncing, isBackgroundSyncing, isManualRefresh, photoReady])
  ...
  if (showFullSyncScreen) {
    return (
      <>
        <MaintenanceOverlay />
        <LoginSyncScreen variant={wasLoginSyncing.current ? "login" : "reload"} />
      </>
    )
  }
