...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/api/admin/feedback`, {
      method: "GET",
      headers: {
    ...
    return NextResponse.json({ success: false, error: "Failed to fetch feedback", feedback: [] }, { status: 500 })
  }
}
