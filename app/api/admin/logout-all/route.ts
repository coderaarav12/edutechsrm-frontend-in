...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/api/admin/logout-all`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ success: false, error: "Failed to logout all sessions" }, { status: 500 })
  }
}
