...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/api/admin/logout-user`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ success: false, error: "Failed to logout user session" }, { status: 500 })
  }
}
