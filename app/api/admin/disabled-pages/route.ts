...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/api/admin/disabled-pages`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ success: false, error: "Failed to disable page" }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: "Failed to re-enable page" }, { status: 500 })
  }
}
