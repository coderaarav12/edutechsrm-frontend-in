...
export function Navbar({ activeTab, setActiveTab, minimised, setMinimised }: NavbarProps) {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      scrollTimeout.current = setTimeout(() => {
        ...
        if (!stillNearFooter) setMinimised(false)
      }, 700)
    }
  ...
  useEffect(() => {
    ...
    const closeOutside = (event: MouseEvent) => {
      if (!(event.target as HTMLElement).closest("[data-nav]")) {
        setMoreOpen(false)
      }
    }
  ...
  const handleOverlayTab = useCallback((tab: TabType) => {
    setMoreOpen(false)
  ...
  return (
    <>
      ...
      {/* ── Mobile: floating active tab indicator ── */}
      <div
        data-nav
      ...
      >
        <motion.button
          ...
          animate={minimised ? { y: 100, opacity: 0 } : { y: 0, opacity: 1 }}
        ...
        >
          <div style={{
            ...
            {(() => {
              ...
              return <Icon style={{ width: "clamp(18px, 4.5vw, 26px)", height: "clamp(18px, 4.5vw, 26px)", color: activeColor }} />
            })()}
          </div>
          ...
          <div style={{
            ...
            borderRadius: "50%",
            background: "var(--element-bg, rgba(255,255,255,0.04))",
            flexShrink: 0,
          }}>
            <svg width="clamp(12px, 3vw, 18px)" height="clamp(12px, 3vw, 18px)" viewBox="0 0 24 24" fill="none" stroke="var(--text-subtle, #71717a)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="m6 9 6 6 6-6" />
            </svg>
          </div>
        </motion.button>
        ...
        <div style={{ position: "relative" }}>
          <motion.button
            animate={minimised ? { y: 100, opacity: 0 } : { y: 0, opacity: 1 }}
            transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }}
            onClick={() => { setShowAiPrompt(false); sessionStorage.setItem("ai_context", "navbar"); handleTab("ai") }}
            style={{
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
          ...
          >
            <Bot style={{ width: "clamp(20px, 5vw, 28px)", height: "clamp(20px, 5vw, 28px)", color: "#a78bfa" }} />
          </motion.button>
      ...
      {/* ── Bottom sheet overlay (mobile/tablet) ── */}
      <AnimatePresence>
        {moreOpen && (
          <>
            <motion.div
              ...
              style={{ background: "color-mix(in srgb, var(--page-bg, #09090b) 80%, transparent)" }}
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "center", paddingTop: "clamp(10px, 2.5vw, 14px)", paddingBottom: "clamp(2px, 0.5vw, 4px)", flexShrink: 0 }}>
                <div style={{ width: "clamp(36px, 8vw, 44px)", height: 4, borderRadius: 2, background: "var(--element-bg, rgba(255,255,255,0.06))" }} />
              </div>
              ...
              <div style={{ overflow: "auto", padding: "clamp(6px, 1.5vw, 12px) clamp(16px, 4vw, 32px) calc(env(safe-area-inset-bottom, 8px) + clamp(10px, 2.5vw, 20px))" }}>
                ...
                <nav style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "clamp(6px, 1.2vw, 12px)", width: "100%" }}>
                  {NAV.filter(item => item.id !== 'updates').map((item) => {
                    ...
                    return (
                      ...
                      >
                        <button
                          onClick={() => handleOverlayTab(item.id)}
                          style={{
                            width: "100%",
                            display: "flex",
                            flexDirection: "column",
                          ...
                          <div
                            style={{
                              ...
                              background: active
                                ...
                                : "var(--element-bg, rgba(255,255,255,0.03))",
                              border: active
                          ...
                          >
                            <item.icon
                              style={{
                                ...
                                color: active ? item.color : "var(--text-faint, #52525b)",
                              }}
                            />
                          </div>
                          <span
                            style={{
                              ...
                              color: active ? item.color : "var(--text-muted, #a1a1aa)",
                              fontFamily: "'Space Grotesk', sans-serif",
                              textAlign: "center",
                              lineHeight: 1.2,
                ...
                {/* Divider */}
                <div style={{ margin: "clamp(12px, 3vw, 20px) 0 clamp(10px, 2.5vw, 16px)", height: 1, background: "linear-gradient(90deg, transparent, var(--border-color, rgba(255,255,255,0.06)), transparent)" }} />

                {/* Bottom row: Updates + Feedback */}
                <div style={{ display: "flex", gap: "clamp(8px, 2vw, 12px)" }}>
                  <button
                    onClick={() => handleOverlayTab("updates")}
                    style={{
                      flex: 1,
                      display: "flex",
                      alignItems: "center",
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(251,191,36,0.1)"; e.currentTarget.style.color = "rgba(251,191,36,0.95)" }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(251,191,36,0.06)"; e.currentTarget.style.color = "rgba(251,191,36,0.7)" }}
                  >
                    <Megaphone style={{ width: "clamp(12px, 2.8vw, 15px)", height: "clamp(12px, 2.8vw, 15px)" }} />
                    Updates
                  </button>
                  <button
                    onClick={() => handleOverlayTab("feedback")}
                    style={{
                      flex: 1,
                      display: "flex",
                      alignItems: "center",
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(167,139,250,0.1)"; e.currentTarget.style.color = "rgba(167,139,250,0.95)" }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(167,139,250,0.06)"; e.currentTarget.style.color = "rgba(167,139,250,0.7)" }}
                  >
                    <MessageSquareText style={{ width: "clamp(12px, 2.8vw, 15px)", height: "clamp(12px, 2.8vw, 15px)" }} />
                    Feedback
                  </button>
                </div>
                ...
                {/* Footer actions */}
                {isAuthenticated ? (
                  <div style={{ display: "flex", gap: "clamp(6px, 1.5vw, 10px)", marginTop: "clamp(24px, 6vw, 40px)", justifyContent: "center" }}
                  >
                    <Link href="/about" onClick={() => setMoreOpen(false)}
                      style={{ display: "flex", alignItems: "center", gap: "clamp(4px, 1vw, 6px)", padding: "clamp(7px, 1.8vw, 11px) clamp(12px, 3vw, 18px)", borderRadius: 999, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", color: "var(--text-muted)", fontSize: "clamp(9px, 2.2vw, 11px)", fontWeight: 700, textDecoration: "none", transition: "background 0.2s, border-color 0.2s" }}
                      ...
                      <IdCard style={{ width: "clamp(11px, 2.5vw, 14px)", height: "clamp(11px, 2.5vw, 14px)" }} />
                      About
                    </Link>
                    <a href="https://academia.srmist.edu.in/" target="_blank" rel="noopener noreferrer" onClick={() => setMoreOpen(false)}
                      style={{ display: "flex", alignItems: "center", gap: "clamp(4px, 1vw, 6px)", padding: "clamp(7px, 1.8vw, 11px) clamp(12px, 3vw, 18px)", borderRadius: 999, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", color: "var(--text-muted)", fontSize: "clamp(9px, 2.2vw, 11px)", fontWeight: 700, textDecoration: "none", transition: "background 0.2s, border-color 0.2s" }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.06)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)" }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)" }}>
                      <ExternalLink style={{ width: "clamp(11px, 2.5vw, 14px)", height: "clamp(11px, 2.5vw, 14px)" }} />
                      Portal
                    </a>
                    <button onClick={() => { setMoreOpen(false); setIsSignOutOpen(true) }}
                      style={{ display: "flex", alignItems: "center", gap: "clamp(4px, 1vw, 6px)", padding: "clamp(7px, 1.8vw, 11px) clamp(12px, 3vw, 18px)", borderRadius: 999, background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.12)", color: "#f87171", fontSize: "clamp(9px, 2.2vw, 11px)", fontWeight: 700, cursor: "pointer", transition: "background 0.2s, border-color 0.2s" }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(248,113,113,0.1)"; e.currentTarget.style.borderColor = "rgba(248,113,113,0.2)" }}
                      ...
                      <LogOut style={{ width: "clamp(11px, 2.5vw, 14px)", height: "clamp(11px, 2.5vw, 14px)" }} />
                      Sign Out
                    </button>
                  </div>
                ) : null}
