...
export function AboutSection() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      fetch(cached).then(r => r.blob()).then(blob => { if (!cancelled) setPhotoUrl(URL.createObjectURL(blob)) }).catch(() => {})
      return
    }
    ...
    fetch("/api/srm/photo", { headers: { "x-access-token": auth.token } })
      ...
      .catch(() => { if (!cancelled) { setPhotoError(true); const cached = readCachedPhoto(auth.token); if (cached) fetch(cached).then(r => r.blob()).then(blob => { setPhotoUrl(URL.createObjectURL(blob)) }).catch(() => {}) } })
