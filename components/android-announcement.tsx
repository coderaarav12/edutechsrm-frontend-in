      ...
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className={`fixed inset-0 z-[70] flex items-center justify-center p-4 backdrop-blur-sm ${
            isPoster ? "bg-black/50" : "bg-black/70"
          }`}
          onClick={onClose}>
          <motion.div initial={{ opacity: 0, y: 14, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 6, scale: 0.98 }}
            ...
            style={
              isPoster
                ? { background: "#ffffff", borderColor: "#111111", borderWidth: "2.5px", borderStyle: "solid", boxShadow: "6px 6px 0px #111111" }
                : { background: "rgba(9,9,11,0.96)", borderColor: "rgba(52,211,153,0.22)", boxShadow: "0 28px 70px rgba(0,0,0,0.6)" }
            }>
            ...
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em]"
                style={
                  isPoster
                    ? { color: "#111111", borderColor: "#111111", background: "#f7f5f0", borderWidth: "1.5px", borderStyle: "solid" }
                    : { color: "#86efac", borderColor: "rgba(52,211,153,0.35)", background: "rgba(52,211,153,0.09)" }
                }>
              ...
              <button onClick={onClose}
                className={`h-7 w-7 rounded-lg flex items-center justify-center transition-colors ${
                  isPoster
                    ? "text-[#111111] bg-[#f7f5f0] border border-[#111111] hover:bg-[#eae6dd]"
            ...
            <div className="mt-5 flex h-16 w-16 items-center justify-center rounded-2xl border"
              style={
                isPoster
                  ? { borderColor: "#111111", borderWidth: "2px", borderStyle: "solid", background: "#f7f5f0", boxShadow: "2px 2px 0px #111111" }
                  : { borderColor: "rgba(52,211,153,0.25)", background: "rgba(52,211,153,0.08)" }
              }>
              <Smartphone className="h-8 w-8" style={{ color: isPoster ? "#111111" : "#34d399" }} />
            </div>

            <h2 className="mt-4 text-2xl font-black tracking-tight" style={{ color: isPoster ? "#111111" : "#f4f4f5" }}>edutechsrm is on Android</h2>
            <p className="mt-2 text-sm leading-6" style={{ color: isPoster ? "#555555" : "#a1a1aa" }}>
            ...
            <div className="mt-6 flex flex-col gap-3">
              <a href="https://play.google.com/store/apps/details?id=in.edutechsrm.app" target="_blank" rel="noopener noreferrer"
                ...
                style={
                  isPoster
                    ? { background: "#111111", color: "#ffffff", border: "2px solid #111111", boxShadow: "3px 3px 0px #111111" }
                    : { background: "linear-gradient(135deg, #34d399, #10b981)", color: "#0b0f14" }
                }>
              ...
              <button onClick={onClose} className="w-full rounded-xl px-4 py-2.5 text-sm font-bold border transition-all"
                style={
                  isPoster
                    ? { color: "#111111", borderColor: "#111111", background: "#f7f5f0", borderWidth: "2px", borderStyle: "solid" }
                    : { color: "#d4d4d8", borderColor: "rgba(255,255,255,0.16)", background: "rgba(255,255,255,0.02)" }
                }>
                Maybe later
              </button>
            </div>
